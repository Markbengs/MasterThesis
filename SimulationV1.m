function [r, varargout] = SimulationV1(V,L,N,S,R,VD,tCircle,wc,draw) %args
%V = 13; R0 = 25; RT = 25; L = 30; N = 10; VD = 1/5; tStraight = 5; tSpiral = 30; tCircle = 30; S = 1; wc = [0;0;0];
mDrouge = 0.5; g = 9.82; e3 = [0;0;1];  rho = 1.225; d = 0.003; %d = 0.00635;
...
for q = 0:h:tStraight
    ...
    if draw == 1 && mod(q,0.02)==0
        plot3(p(1,:),p(2,:),p(3,:))
        set(gca,'Ydir','reverse','Zdir','reverse')
        axis([-tStraight*V 30 -R-1 R+1 -5 35])
        %view(180,0)
        drawnow
    end
end
...
for q = 0:h:tCircle
    p(:,1) = [R*sin(V*q/R);R*cos(V*q/R); q*VD];
    Dp(:,1) = [V*cos(V*q/R); -V*sin(V*q/R); VD];
    DDpm = [-V^2*sin(V*q/R)/R; -V^2*cos(V*q/R)/R; 0];
    ...
    if draw == 1 && mod(q,0.1) == 0
        plot3(p(1,:),p(2,:),p(3,:))
        set(gca,'Ydir','reverse','Zdir','reverse')
        axis([ -30 30 -R-1 R+1 -5 35]) %-tStraight*V
        %view(0,90)
        drawnow
    end
    
    if q > tCircle-tLastLap+1e-10
    %test(:,round((q-tSpiral)/h)+1) = p(:,end);
    test(:,round((q-tCircle+tLastLap)/h)) = p(:,end);
    end
end
...
zfix = test(3,:)-(0:h:tLastLap-h)*VD;
%plot3(test(1,:),test(2,:),zfix)
